...
\startsubsection[title={Predecessors}]
    ...
    \startformula
        \RobustPredecessor{\StateSpace'}{\ControlSpace'}{\IndexedSet{\State{j}}{j \in J}} = \bigcup_{\startsubstack \NC J' \subseteq J \NR \NC  J' \ne \emptyset \NR \stopsubstack} \PrecisePredecessor{\StateSpace'}{\ControlSpace'}{\IndexedSet{\State{j}}{j \in J'}} \EndPeriod
    \stopformula

    The computation of the precise predecessor ... % TODO
...
\startsubsection[title={Actions}]
    ...
    This means e.g.\ that it is possible that for some $\VecState \in \Predecessor{\StateRegion}{\ControlSpace}{\StateTarget}$ $\Action{\VecState}{\StateTarget} = \emptyset$, even though this should be true theoretically.
    % TODO justify omission of lower-dimensional actions for practical purposes
    ...
    There is an important difference in how the conditions of $\PreP$ and $\ActC$ work, that should be stressed:
    ...
    \startformula
        \startalign[n=2,align={right,left}]
            ...
            \NC \bigcap_{j \in J} \Action{\StateRegion}{\State{j}} \setminus \bigcup_{i \in I \setminus J} \Action{\StateRegion}{\State{i}} \EndPeriod
            \NR
        \stopalign
    \stopformula
