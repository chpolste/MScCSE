...
where the 4 linear predicates $\Predicate_{1}$, $\Predicate_{2}$, $\Predicate_{3}$ and $\Predicate_{4}$ are specified, corresponding to halfspaces to halfspaces governed by the inequalities $x \leq 1$, $-x \leq 1$, $y \leq 1$ and $-y \leq 1$.
The co-safe interpretation of the objective is chosen.
The initial state space decomposition and the extended state space according to $\Posterior{\StateSpace}{\ControlSpace}$ is visualized in \in{Figure}[fig:cases-integrator-initial].
...
\startsubsection[title={Negative Refinement},reference=sec:cases-integrator-negative]
    ...
    Reported are the elapsed time since problem initialization, the number of polytopes in the current state space partition, the game size in player 1 and 2 states and actions, the elapsed time during the refinement application, game graph construction (abstraction) and analysis (game solution) as well as the volume fraction of regions $\YesStates{q_0}$ (yes), $\NoStates{q_0}$ (no) and $\MaybeStates{q_0}$ (maybe).
