...
\startsubsection[title={Operations on Convex Polytopes},reference=sec:theory-geometry-operations]
    ...
    Aside from the usual set operations like intersection, union and difference, two additional binary operations are defined for polytopes $X, Y \in \reals^n$. The Minkowski sum
    ...
    This is illustrated in Figure \in[fig:theory-geometry-operations].
    Furthermore, $X \ominus Y \neq X \oplus (-Y)$ in general.

    \placefigure[top][fig:theory-geometry-operations]{
        Illustration of the Pontryagin difference (center) and Minkowski sum (right).
        Note that $(Z \oplus Y) \ominus Y = Z$, but $(X \ominus Y) \oplus Y = Z \oplus Y \neq X$.
