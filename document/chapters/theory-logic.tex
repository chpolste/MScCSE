...
\startsubsection[title={Fragments of LTL}]
    ...
    This is advantageous e.g. when dealing with problems where some accumulated cost is supposed to be minimized.
    ...
    Co-safe formulae also correspond to many real-world scenarios, e.g. in robotics where the objectives of some robot are usually not actually infinite procedures or allowed to take infinite time, but isolated, finite tasks that are chained to form more complex behaviour.
    ...
    The class of (extended) Generalized Reactivity(1) formulae is a popular fragment that offers polynomical computational complexity while retaining a high degree of expressivity. % TODO: references of users, introduce standard GR(1) first
    ...
    where every \math{\mu_i} and \math{\pi_i} is representable by a deterministic \omega-automaton with Büchi acceptance condition. % TODO connection to automata in next section, swap sections
    Here, when talking about GR(1), the extended framework is always meant. % TODO phrasing
...
% TODO FGx automaton with empty set acceptance
...
\startsubsection[title={Automata for LTL Objectives}]
    ...
    % TODO translation is hard, different types of automata, acceptance conditions, non-uniqueness

    % TODO rewrite this after reordering the sections
