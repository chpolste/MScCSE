...
\startformula
    \VecX_{t+1} = \TwoByTwo{1}{0}{0}{1} \VecState_{t} + \TwoByTwo{1}{0}{0}{1} \VecControl_{t} + \VecRandom_{t}
\stopformula
...
\startsubsection[title={Reachability Analysis},reference=sec:cases-corridor-reachability]
    ...
    Both run time and the size of the product game measured in player actions are similar to that of the double integrator solution in \in{Table}[tab:cases-integrator-layers-prer100] despite a state space partition that has only a third of the number of polytopes.
...
\startsubsection[title={Reachability Controller}]
    ...
    Using the round-robin scheme introduced in \in{section}[sec:abstraction-analysis-synthesis], a strategy is synthesized from the solution of the reachability analysis problem (\in{Table}[tab:cases-corridor-reachability]).
    ...
    It has no problem guaranteeing the safety of traces but in order to navigate the corridor, the trace evolution has to be controlled by \quotation{steering} to the right for multiple timesteps in sequence.
...
\startsubsection[title={2-Recurrence and Safety},reference=sec:cases-corridor-recurrence]
    ...
    Compared to the reachability solution (\in{Table}[tab:cases-corridor-reachability]), the number of polytopes in the partition has approximately doubled which could be expected considering that both rooms were refined instead of just the right one.
