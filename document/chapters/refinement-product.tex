...
\startbuffer[buf:refinement-product-negattr-algorithm]
...
\startsubsection[title={Negative Attractor},reference=sec:refinement-product-negattr]

    \placealgorithm[top][alg:refinement-product-negattr]{
        ...
        \startframedtext[width=\textwidth,frame=off]
            \getbuffer[buf:refinement-product-negattr-algorithm]
        \stopframedtext
    }

    \cite[Svorenova2017] identify the pattern

    \placeformula[fml:refinement-product-negattr-condition]
    \startformula
    ...
    A procedure based on the negative refinement of \cite[Svorenova2017] that refines an element of the state space partition with respect to the attractor region, taking into account every associated product state is given by Algorithm \in[alg:refinement-product-attractor].
    ...
    where $R_n$ is the set of all (full-dimensional) polytopic regions in $\reals^n$ and $C_n$ is the set of all convex polytopes in $\reals^n$, which partitions any polytopic region into a set of (disjunct) convex polytopes.
...
\startsubsection[title={Safety}]
    ...
    Algorithm (\in[alg:refinement-product-safety]) describes a procedure with the goal of making a state safe in the above sense by splitting the associated polytope of a player 1 state matching the pattern (\in[fml:refinement-product-negattr-condition]) into parts where player 1 actions exist that guarantee a safe evolution of the LSS.
    It relies on a one-step positive refinement procedure

    \startformula
        \Function{\RefinePos}{C_n \times R_n}{2^{C_n}} \EndComma
    \stopformula

    where $C_n$ and $R_n$ are defined as in section \in[sec:refinement-product-negattr].
    ...
    If no safe subregion of $X$ can be determined, $\RefinePos$ may either partition $X$ arbitrarily or not at all.
    If $Y$ can already be reached in the above manner from every state in $X$, $X$ is returned unchanged.
    The robust attractor kernel introduced in section \in[sec:refinement-reduction-robustoperators] a procedure matching these criteria.
        ...
        \FORALL{$q \in Q$}
            \IF{$\State{i} \in \MaybeStates{q} \MidAnd \QNext{i}{q} = q$}
                \IF{$\exists J \ne \Set{i} : ( \PlayerTwoAction{i}{J}{\Set{i}} \in Act \MidAnd \IndexedStates{j}{J} \cap \NoStates{q} = \emptyset )$}
                    ...
                    \STATE{$A \leftarrow \ConcreteAction{\State{i}}{\IndexedStates{j}{J}}$}
...
\startsubsection[title={Loop Removal},reference={sec:refinement-product-loops}]
    ...
    The refinement procedure in Algorithm \in[alg:refinement-product-loops] is proposed to remove potentially problematic self-loops in the game graph.
