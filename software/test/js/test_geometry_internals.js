...
describe("geometry internals", function () {
    ...
    it ("angleCCW yields (2π - angle) for reverse argument order", function () {
        ...
        const h1 = new geom.Halfspace([-1, 0], -1);
        const h2 = new geom.Halfspace([-1, 2.1981294421572875e-15], -1);
    ...
    it("halfplaneIntersection", function () {
        let h1 = geom.Halfspace.normalized([1, 0], 0);
        let h2 = geom.Halfspace.normalized([0, 1], 0);
        let h3 = geom.Halfspace.normalized([1, 0], 1);
        let h4 = geom.Halfspace.normalized([0, 1], 1);
